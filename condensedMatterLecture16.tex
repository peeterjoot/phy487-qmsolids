...
Assume a periodic lattice with large lattice parameter \(a\), so that atomic potential \(V_a(\Br - \Br_n)\) and wave functions \(\phi(\Br- \Br_n)\) don't overlap much between neighbours, as sketched in \cref{fig:qmSolidsL16:qmSolidsL16Fig1}.  The potential \(\phi_i(\Br - \Br_n)\) satisfies
%
