...
on neighboring atoms, to visualize how they overlap.
} % makeproblem
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:160}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:180}
\Br_m - \Br \in \{(0, a), (0, -a), (a, 0), (-a, 0)\}.
\end{equation}
%
The sum of exponentials is just
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:200}
\begin{aligned}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:220}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:20}
\Bk(u) = \frac{2 \pi}{a} u (1, 0),
\end{equation}
%
so the energy on this trajectory is
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:40}
\begin{aligned}
E(\Bk(u))
&=
E_s - A - 2 B \lr{
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:60}
\Bk(v) = \frac{2 \pi}{a} v (0, 1),
\end{equation}
%
so the energy on this trajectory is
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:80}
\begin{aligned}
E(\Bk(v))
&=
E_s - A - 2 B \lr{
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:100}
\Bk(w) = \frac{2 \pi}{a} w (1, 1),
\end{equation}
%
so the energy on this trajectory is
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:120}
\begin{aligned}
E(\Bk(w))
&=
E_s - A - 2 B \lr{
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:140}
4 B - (-4B) = 8B.
\end{equation}
%
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:240}
E = E_s - A - 2 B \lr{ \cos k_x a + \cos k_y a } =
E_s - A - 2 B,
\end{equation}
%
or
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:260}
\cos k_x a + \cos k_y a = 1.
\end{equation}
%
\paragraph{(ii) \(E = E_s - A - B\)} Next we have the contour defined by
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:280}
\cos k_x a + \cos k_y a = \inv{2}.
\end{equation}
%
\paragraph{(iii) \(E = E_s - A\)} This contour is defined by
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:300}
\cos k_x a + \cos k_y a = 0.
\end{equation}
%
\paragraph{(iv)  \(E = E_s - A + 2B\)} And the last contour defined by
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:320}
\cos k_x a + \cos k_y a = -1.
\end{equation}
%
...
``Unlike the calculation in problem set 8, the band structure of the alkali metals and alkali earths is free-electron-like.  This means that the dispersion relation is parabolic, except near the Brillouin zone boundary.  So if the Fermi surface never gets close to a Brillouin zone boundary,
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:340}
...
%
\paragraph{Grading remarks:} \(x \pm a\) is circled ``be careful with this.  Need to presume \textunderline{parity} of orbital'', and ``\(B_x\) is \textunderline{negative}''.
%
Putting the origin at \(\Br_n\), writing \(\alpha = Z/a_\nought\), and \(c^2 = (Z/2 a_\nought)^5/\pi\), the \(A\) and \(B\) integrals for this basis are
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:360}
\begin{aligned}
...
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:380}
\begin{aligned}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:420}
E(\Bk) \approx E_s - \beta(2) - 2 \beta(1) e^{- Z a/a_\nought} \lr{ \cos k_x a + \cos k_y a},
\end{equation}
%
where
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:440}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:460}
E = E_s - E_s - \beta(2) - 2 \beta(1) \mu.
\end{equation}
%
the contours are defined by
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:480}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:520}
\begin{aligned}
E(\Bk) &=
E_s
+ c^2 \int d^3 \Br v(\Br) e^{-2 \alpha r} x^2  \\
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:540}
E(\Bk) = E_s - A
+ \Abs{B_x} \cos k_x a
- \Abs{B_y} \cos k_y a.
\end{equation}
...
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:560}
E(\Bk) = E_s - A - 2 \mu \sqrt{ B_x^2 + B_y^2 },
\end{equation}
%
so that the constant surfaces are given by
%
\begin{equation}\label{eqn:condensedMatterProblemSet8Problem1:580}
-\cos k_x a
