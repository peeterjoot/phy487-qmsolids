...
%
\begin{equation}\label{eqn:diffractionReflection:20}
...
%
\begin{equation}\label{eqn:diffractionReflection:40}
...
%
\begin{equation}\label{eqn:diffractionReflection:60}
\begin{aligned}
\Abs{\Br - \BR}
&=
\sqrt{(\Br - \BR)^2}
...
%
\begin{equation}\label{eqn:diffractionReflection:80}
\begin{aligned}
U(\BR')
&\sim
\frac{U_\nought }{R R'}
...
%
Finally write
\begin{equation}\label{eqn:diffractionReflection:100}
\begin{aligned}
\Bk_s &= - k\Rcap \\
\Bk &= k\Rcap',
\end{aligned}
...
for
%
\begin{equation}\label{eqn:diffractionReflection:120}
U(\BR')
\sim
\frac{U_\nought }{R R'}
e^{i k (R + R')}
...
Finally, write
%
\begin{equation}\label{eqn:diffractionReflection:140}
\BK = \Bk - \Bk_s,
\end{equation}
%
and introduce an aperture function \(\rho(\Br)\) (we called this the \textAndIndex{scattering density}).  This aperture function is unity for any regions of the aperture that light is allowed through, and zero when light is blocked, and some value in \([0,1]\) for translucent regions of the aperture.  We can now expand the integral over the surface containing the aperture
